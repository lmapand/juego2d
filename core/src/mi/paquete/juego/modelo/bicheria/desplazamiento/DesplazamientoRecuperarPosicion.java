...
public class DesplazamientoRecuperarPosicion implements Desplazable
{
    ...
    public DesplazamientoRecuperarPosicion(SerFisico bicho, CoordCasilla destino)
    {
        ...
        Gdx.app.log("CREANDO DESP RECUPERACION", "Dirigiendose a la posicion "+destino.x+","+destino.y);
        ...
        if(Analizador.distancia(bicho.getPosX(), bicho.getPosY(), posCentroCasillaX,posCentroCasillaY)<=radioCorte)
        {
            ...
            //Situacion en la que RecuperarPosicion es una sola casilla
        ...
        else
        {
            ...
            Gdx.app.log("DESP RECUPERAR POS:", String.format("Yendo de %5.0f,%5.0f a %5.0f,%5.0f   %3.2f",bicho.getPosX(), bicho.getPosY(),destinoX, destinoY, Math.toDegrees(anguloAcercamiento)));
    ...
    /** funcion auxiliar que realiza el centrado del bicho, para evitar problemas de posicionamiento.
     *
     */
    public void centra()
    {
        Mundo mundo=Mundo.getInstance();
        bicho.setPosX(bicho.getCasillaActual().x* mundo.getAnchoCasilla() + mundo.getAnchoCasilla() / 2);
        bicho.setPosY(bicho.getCasillaActual().y* mundo.getAltoCasilla() + mundo.getAltoCasilla() / 2);
    }
